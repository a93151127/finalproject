...
<form action="Controller" method="post">
    <table width="100%" border="0" align="center" class="threeboder">
        ...
        <c:forEach var="row" items="${cart}">
            <tr>
                <td height="50" align="left" class="trow">&nbsp;&nbsp;${row.productName}</td>
                <td align="center" class="trow">
                     <input name="quantity_${row.goodsid}" type="text" value="${row.quantityOrder}"
                           onblur="calc(${row.goodsid}, this)"/>
                </td>
                <td align="center" class="trow"><span id="price_${row.goodsid}">${row.buyPrice}</span></td>
                <td align="center" class="trow"><span
                        id="subtotal_${row.goodsid}">${row.buyPrice * row.quantityOrder}</span>
                </td>
            </tr>
        </c:forEach>
